...
import { AlertTriangle, BrainCircuit, Check, Copy, Cpu, Clock, Terminal } from "lucide-react";
...
export default function IncidentDetailPage() {
  ...
  const handleAnalyze = () => {
    analyzeMutation.mutate({ id }, {
      onSuccess: () => {
        toast({ title: "Analysis Complete", description: "AI root cause analysis updated." });
      }
    });
  };
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="glass-card rounded-xl p-6 border border-primary/20 neon-border-blue relative overflow-hidden">
              ...
              <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-primary">
                <BrainCircuit className="h-5 w-5" /> AI Root Cause Analysis
              </h2>
              ...
              {isIncidentLoading ? (
                ...
                <div className="space-y-6 relative z-10">
                  <div>
                    ...
                    <p className="text-foreground leading-relaxed">{incident?.rootCause || "Analysis pending."}</p>
                  </div>
                  {incident?.aiAnalysis && (
                    <div>
                      <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Detailed Explanation</h3>
                      <p className="text-muted-foreground leading-relaxed">{incident.aiAnalysis}</p>
                    </div>
                  )}
                  {incident?.confidence && (
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Confidence Score:</span>
                      <span className="text-primary font-mono">{incident.confidence}%</span>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="glass-card rounded-xl border border-border overflow-hidden">
              <div className="bg-muted/30 px-4 py-3 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm">Suggested Remediation Commands</span>
                </div>
              </div>
              <div className="p-4 bg-black/90 font-mono text-sm space-y-2">
                ...
                {!incident?.suggestedCommands?.length && (
                  <div className="text-muted-foreground italic">No commands suggested.</div>
                )}
              </div>
            </div>
          </div>
                ...
                {isTimelineLoading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] glass-card p-4 rounded-xl border border-border">
                  ...
                  <div key={event.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full border bg-background shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 ${
                      event.type === 'DETECTION' ? 'border-red-500/50 text-red-500' :
                      event.type === 'RESOLUTION' ? 'border-green-500/50 text-green-500' :
